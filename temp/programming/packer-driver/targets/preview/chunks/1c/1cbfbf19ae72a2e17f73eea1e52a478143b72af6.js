    ...
    execute: function () {
      ...
      _export("GameC", GameC = (_dec = ccclass("GameC"), _dec2 = property({
        ...
        start() {
          ...
          var userInfo = this.getUsernameAndBalanceFromLocalStorage(this.loggedInUsername);
        ...
        createGrid() {
          ...
          var col = Math.floor(this.numPrefabsCreated / this.numRows);
          var row = this.numPrefabsCreated % this.numRows;

          if (!this.grid[row]) {
            this.grid[row] = [];
          }

          var prevRowPrefabType = null;
          var prefabType = this.sum <= 10 ? this.Under : this.Over;
          ...
          var cell = instantiate(prefabType); // Kiểm tra xem prefab không phải là prefab đầu tiên trong cột
        ...
        bet() {
          if (this.checkOver === true && this.cloneBalanUser >= this.totalUserBetOver && this.balanceUser !== 0 && this.gameEnd === false) {
            ...
            var isUpdated = this.setUsernameAndBalanceToLocalStorage(this.loggedInUsername, this.balanceUser);
            ...
            var _isUpdated = this.setUsernameAndBalanceToLocalStorage(this.loggedInUsername, this.balanceUser);
        ...
        scheduleRandomDice() {
          ...
          setTimeout(() => {
            ...
            for (var i = 0; i < this.tkpTime.length; i++) {
            ...
            this.plate.getComponent(Animation).on(Animation.EventType.FINISHED, () => {
              ...
              if (this.sum < 10) {
                ...
                var _isUpdated2 = this.setUsernameAndBalanceToLocalStorage(this.loggedInUsername, this.balanceUser);
        ...
        getUsernameAndBalanceFromLocalStorage(username) {
          ...
          if (storedAccounts) {
            var registeredAccounts = JSON.parse(storedAccounts);
            var account = registeredAccounts.find(acc => acc.username === username);
        ...
        setUsernameAndBalanceToLocalStorage(username, newBalance) {
          ...
          if (storedAccounts) {
            var registeredAccounts = JSON.parse(storedAccounts);
            var index = registeredAccounts.findIndex(acc => acc.username === username);
...
//# sourceMappingURL=1cbfbf19ae72a2e17f73eea1e52a478143b72af6.js.map